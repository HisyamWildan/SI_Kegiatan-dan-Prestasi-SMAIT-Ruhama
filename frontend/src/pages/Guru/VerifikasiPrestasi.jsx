...
import { Table, Button, Modal, Badge, Row, Col, Form, Carousel } from 'react-bootstrap';
...
const VerifikasiPrestasi = () => {
    ...
    const handleRejectSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.put(`/prestasi/${selectedItem.id}/reject`, {
                rejection_message: rejectionMessage
            });
            fetchPendingPrestasi();
            handleClose();
            alert('Prestasi telah ditolak dengan alasan.');
        } catch (error) {
            console.error(error);
            alert('Gagal menolak prestasi');
        }
    };
    ...
    return (
        <GlobalVerifikasiStyles>
            <div className="pb-5">
                ...
                <Modal show={show} onHide={handleClose} size="xl" centered scrollable className="rounded-4">
                    <Modal.Header closeButton className="border-0 px-4 pt-4">
                        <Modal.Title className="fw-bold text-dark">Detail Verifikasi Prestasi</Modal.Title>
                    </Modal.Header>
                    ...
                    <Modal.Footer className="border-0 px-4 pb-4 gap-2">
                        ...
                        <div className="ms-auto d-flex gap-2">
                            <Button variant="outline-danger" className="px-4 py-2 rounded-3 fw-bold" onClick={() => setShowRejectModal(true)}>Tolak</Button>
                ...
                <Modal show={showRejectModal} onHide={() => setShowRejectModal(false)} centered className="rounded-4">
                    <Form onSubmit={handleRejectSubmit}>
                        <Modal.Header closeButton className="border-0 px-4 pt-4">
                            <Modal.Title className="fw-bold text-danger">Alasan Penolakan</Modal.Title>
                        </Modal.Header>
                        <Modal.Body className="px-4">
                            <Form.Group>
                                ...
                                <Form.Control 
                                    ...
                                    rows={4} 
                                    required 
                                    placeholder="Jelaskan alasan penolakan atau apa yang perlu diperbaiki (Misal: Foto bukti kurang jelas)"
                        ...
                        <Modal.Footer className="border-0 px-4 pb-4">
                            <Button variant="light" className="fw-bold" onClick={() => setShowRejectModal(false)}>Batal</Button>
                            <Button variant="danger" type="submit" className="fw-bold px-4">Kirim Penolakan</Button>
                        </Modal.Footer>
                    </Form>
                </Modal>
            </div>
